...
import { STACK_NAME_RE, readStack, stackExists, writeStack } from './stacks.js';
...
describe('writeStack / readStack', () => {
  it('rejects a compose file without a services key', async () => {
    await expect(writeStack('bad-stack', 'foo: bar')).rejects.toThrow(
      'The compose file must define at least "services"'
    );
  });

  it('rejects a document that is not a YAML mapping', async () => {
    await expect(writeStack('bad-stack-2', 'just a string')).rejects.toThrow();
  });

  it('writes and reads back a valid compose file', async () => {
    const compose = 'services:\n  web:\n    image: nginx:alpine\n';
    await writeStack('good-stack', compose);
    expect(await stackExists('good-stack')).toBe(true);
    expect(await readStack('good-stack')).toBe(compose);
  });

  it('reports a stack as not existing until it has been written', async () => {
    expect(await stackExists('never-written')).toBe(false);
  });
});
