...
describe('POST /api/hosts/local/containers', () => {
  it('rejects a request without an image', async () => {
    const { agent } = await createAdminAgent(app);
    const res = await agent.post('/api/hosts/local/containers').send({});
    expect(res.status).toBe(400);
  });

  it('creates and starts a container on the happy path', async () => {
    const { agent } = await createAdminAgent(app);
    const res = await agent.post('/api/hosts/local/containers').send({ image: 'alpine:latest' });
  ...
  it('rejects auto-remove combined with a non-default restart policy', async () => {
    ...
    const res = await agent
      .post('/api/hosts/local/containers')
  ...
  it('builds host config from the advanced settings fields', async () => {
    const { agent } = await createAdminAgent(app);
    await agent.post('/api/hosts/local/containers').send({
      image: 'alpine:latest',
      command: ['sleep', '300'],
      workingDir: '/app',
      labels: ['team=infra'],
  ...
  describe('resource quotas for non-admin users', () => {
    it('is never capped for an admin, even with a quota configured', async () => {
      ...
      await agent.post('/api/hosts/local/containers').send({ image: 'alpine:latest', memoryMb: 4096, cpus: 8 });
      const options = mockDocker.createContainer.mock.calls[0][0];
    ...
    it('rejects a non-admin request that exceeds the configured quota', async () => {
      ...
      const res = await agent.post('/api/hosts/local/containers').send({ image: 'alpine:latest', memoryMb: 512 });
      expect(res.status).toBe(400);
      expect(mockDocker.createContainer).not.toHaveBeenCalled();
    });
    ...
    it('allows a non-admin request at or under the quota', async () => {
      ...
      const res = await agent.post('/api/hosts/local/containers').send({ image: 'alpine:latest', memoryMb: 128 });
      expect(res.status).toBe(201);
      const options = mockDocker.createContainer.mock.calls[0][0];
      expect(options.HostConfig.Memory).toBe(128 * 1024 * 1024);
    });
...
describe('POST /api/hosts/local/containers/:id/actions/:action', () => {
  it('rejects an unknown action', async () => {
    const { agent } = await createAdminAgent(app);
    const res = await agent.post('/api/hosts/local/containers/container-123/actions/nuke');
    expect(res.status).toBe(400);
  });
...
describe('DELETE /api/hosts/local/containers/:id', () => {
...
describe('GET /api/hosts/local/containers/:id/logs', () => {
  it('demultiplexes the raw Docker log stream', async () => {
    ...
    const { agent } = await createAdminAgent(app);
    const res = await agent.get('/api/hosts/local/containers/container-123/logs');
    expect(res.status).toBe(200);
